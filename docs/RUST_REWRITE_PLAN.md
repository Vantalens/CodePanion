...
| 二进制+依赖 | ~200MB (node_modules) | < 100MB | **-100MB** |

### Node.js 资源占用热点

1. **Node.js 运行时开销**（最大热点 60-80%）
   - V8 引擎 + 事件循环：30-50MB
...
3. **WebSocket (ws) 开销**（中等热点）
   - 内存：~10-20MB（每个连接 + 缓冲区）
   - JavaScript 层的帧解析效率不高

### Rust 重构预期收益

| 指标 | Node.js | Rust | 改善 |
|------|---------|------|------|
| daemon 空闲内存 | 80-120MB | **11-30MB** | **-60~-90MB (-75~-90%)** ✅ |
| 运行 1 个 workflow | 200-400MB | **150-250MB** | **-50~-150MB (-25~-38%)** |
| daemon 冷启动 | 800-1200ms | **200-500ms** | **-300~-1000ms (-37~-83%)** ✅ |
| daemon 热启动 | 200-400ms | **50-100ms** | **-100~-350ms (-50~-88%)** |
| daemon 二进制 | N/A | **< 20MB** | 无运行时依赖 ✅ |
| workflow 启动延迟 | < 50ms | **< 20ms** | **-30ms (-60%)** |

**实际达成**（P7-04 基准测试）：
- ✅ daemon 空闲内存：**11.82 MB**（超预期 4x）
- ✅ daemon 二进制：**3.98 MB**（超预期 5x）
- ⚠️ daemon 冷启动：~823ms（可优化到 < 500ms）

### 性能目标

#### 内存占用
- daemon 空闲：< 50MB（**已达成 11.82MB**）
- daemon 运行 1 个 workflow：< 150MB
- daemon 运行 3 个 workflow：< 300MB

#### 硬盘占用
- daemon 二进制：< 20MB（**已达成 3.98MB**）
- 安装包（压缩）：< 30MB
- 安装后：< 100MB

#### 启动时间
- daemon 冷启动：< 500ms（当前 ~823ms，待优化）
- daemon 热启动：< 100ms
...
**开发投入**：10-16 天（实际 P7 进度：~95% 完成）

**收益**：
- 内存节省：**-90%**（超预期）
- 启动提升：**2-3x**（可进一步优化）
- 二进制大小：**3.98 MB**（无运行时依赖）
- 用户体验：显著提升（启动快、占用低）
