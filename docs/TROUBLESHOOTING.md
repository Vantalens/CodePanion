...
RemindAI 的 HTTP、WebSocket、GUI 日志和 WebView 消息都应使用 UTF-8。若中文标题或正文显示乱码：

1. 确认调用方请求头包含 `Content-Type: application/json; charset=utf-8`。
2. 使用 `remindai notify "测试通知" -m "这是一条中文消息"` 验证 daemon 到 GUI 的链路。
3. 检查 `~/.remindai/gui.log` 和 daemon 日志是否已经乱码；如果日志正常但页面乱码，问题在 WebView 渲染层。
...
1. `remindai status` 确认 daemon 正在运行。
2. 检查 `~/.remindai/config.json` 中 GUI 和扩展使用的 `port`、`token` 是否一致。
3. 发送 `remindai notify "测试通知" -m "中文消息"`。
4. GUI 未打开时，至少应看到系统通知或 daemon warning。
5. GUI 打开时，时间线应出现 `notification` 消息。

## 多源监控没有事件

1. VS Code 扩展需要加载 `packages/vscode-extension/`，并能读取 `~/.remindai/config.json` 或手动配置 token。
2. 浏览器扩展需要在选项页配置 token，并把目标域名加入 allowlist。
3. 外部适配器应先调用 `POST /sources/register`，再调用 `POST /events`。
...
应该显示类似：
```
[remindai] daemon running (pid=27160, port=7777)
```
...
# 关闭 GUI（如果正在运行）
taskkill //F //IM RemindAI.Gui.exe
...
### 连接流程
1. GUI 启动时读取 `~/.remindai/config.json`
...
1. `remindai status` 的输出
2. `cat ~/.remindai/config.json` 的内容
