...
RemindAI 是一个本地优先、供应商中立、单入口多出口的 AI 开发工作流控制台 / 控制平面。它服务于已经在本机同时使用多个 AI 编程工具的开发者，把终端、IDE、独立 AI 编辑器和本地适配器中的会话、状态、等待输入、结果和上下文统一收束到一个可直接交互的图形工作台中。

现有产品不推倒重做。当前的 Windows GUI、本地 daemon、CLI/PTTY、Codex Desktop 同步、VS Code 来源注册、外部适配器 API 和统一 workflow 模型，都是控制平面路线的基础资产。

## 市场判断与差异化

AI 编程已经进入多会话、多工具、多模型并存阶段。RemindAI 不和 Cursor、Trae、Windsurf、VS Code/Copilot 这类 AI IDE 正面竞争，也不做模型聊天客户端；它要做的是比简单工具切换器更强的本地工作流控制层。
...
- **保留当前入口**：Windows Alpha 继续以 `RemindAI.Gui.exe` 双击运行为普通用户路径。
...
- 不把 RemindAI 定位为 Codex、Claude Code、VS Code、Cursor、Trae 或 CodeBuddy 的替代品
