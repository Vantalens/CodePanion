...
CodePanion 是一个本地优先、供应商中立、单入口多出口的 AI 编程工作流中控台 / 控制平面。它服务于已经在本机同时使用多个 AI 编程工具的开发者，把终端、IDE、独立 AI 编辑器和本地适配器中的会话、状态、等待输入、结果和上下文统一收束到一个可直接交互的图形工作台中。仓库内具体措辞以 [产品定位契约](POSITIONING.md) 为准。

现有产品不推倒重做。当前的 Windows GUI、本地 daemon、CLI/PTTY、Codex Desktop 同步、VS Code 来源注册、外部适配器 API 和统一 workflow 模型，都是控制平面路线的基础资产。

## 市场判断与差异化

AI 编程已经进入多会话、多工具、多模型并存阶段。CodePanion 不和 Cursor、Trae、Windsurf、VS Code/Copilot 这类 AI IDE 正面竞争，也不做 OpenClaw 类通用个人 Agent、Raycast 式通用 launcher、Activity Monitor 式进程监控器或模型聊天客户端；它要做的是比简单工具切换器更强、比单厂工作台更中立的本地 AI 编程任务控制层。
...
5. **任务单元优先**：围绕等待输入、失败、运行中、需审阅和完成状态组织体验，而不是围绕聊天、模型或系统进程组织体验。
