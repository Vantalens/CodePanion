...
- [-] **W-20** 将主界面第一层重排为 workspace / workflow 列表，而不是外部会话列表。
  ...
  - board 上点 workflow 卡片「启动」直接 POST `/workflow/runs`，点 gate 卡片 approve / retry / reject 直接 POST `/workflow/gates/.../resolve`（对应 W-32 三种决策），动作成功后 host 自动重拉 board
  - 待办：gate 决策抽屉收集 constraints / message、按 workspace 切换、把 W-31 step output 摘要在 run 卡片里展开
