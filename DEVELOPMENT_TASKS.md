...
## 2026-05-27 第四轮审计修复完成

来源：[docs/CODE_REVIEW_2026-05-27.md](docs/CODE_REVIEW_2026-05-27.md)（第三轮修复后的回归与 Alpha 收口审查）。执行方案见 [docs/superpowers/plans/2026-05-27-alpha-stabilization-plan.md](docs/superpowers/plans/2026-05-27-alpha-stabilization-plan.md)。

### P1：先恢复可靠门禁与 native 信任边界

- [x] **R-1** `server.integration.test.mjs` 使用 `port: 0` 后通过 `fetch` 请求，随机踩 Fetch blocked port，`npm test` 偶发 `Error: bad port` → 测试 HTTP helper 已改用 `node:http`；整套测试通过且集成测试连续运行 10 次无波动。
- [x] **R-2** WebView2 host 与前端 click 路径显式放行任意 `data:` URI → native 现只允许 `https://codepanion.local/` 与精确 `about:blank`，前端将 `data:` 转交 host 拒绝，并有回归测试覆盖。

### P2：完成 Alpha 验收与叙事收束

- [x] **R-3** S-2 必须复制 `daemon/node_modules`，但 Alpha 验收仍要求产物不含 `node_modules` → 已改为 runtime allowlist；打包时剔除 source / map / PDB / 测试 / 示例 / 编译期依赖等开发资料，并自动执行依赖加载及目录门禁。
- [x] **R-4** `package.json`、daemon package 与当前文档入口仍使用旧 `AI coding workflow control plane` / “控制平面”产品口径 → 用户可见描述已统一到“跨软件 / 跨窗口 / 跨项目的多任务完整操作台”，内部技术表述改为事件协议。
- [x] **R-5** 新增的 `config.json` 损坏隔离逻辑未有直接测试 → `loadConfigFromPath` 已提供临时路径测试入口，并覆盖 JSON parse / schema 失败后的隔离和默认配置重建。
...
- [x] **N-19 / R-2** WebView2 导航边界 → 已订阅 `NavigationStarting` / `NewWindowRequested`，仅放行 `https://codepanion.local/` 与精确 `about:blank`；外链走 `OpenExternalLink` 确认后由系统浏览器打开，`data:` 及其它非 http(s) 协议直接拒绝。
...
- [ ] **打包产物入口审查**：`scripts/package-windows.ps1` 输出的便携目录只将 `CodePanion.Gui.exe`、`README_START.txt` 暴露为用户入口；必要的 `daemon/node_modules` 仅允许 runtime allowlist，不含源码、编译期依赖、source map、PDB、测试、示例和多余平台 prebuild。
  ...
  - [x] `scripts/validate-portable-package.ps1` 已自动核对 runtime allowlist、开发产物剔除、唯一目标平台 prebuild 和 packaged Node 依赖加载。
  - [ ] 真机启动前复核目录树和入口呈现，把截图或录屏证据写入日志。
