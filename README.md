...
- **多源工作流汇聚**：统一接收 CLI/PTTY、Codex Desktop、本地 AI 工具扫描、CC Switch、VS Code 扩展和外部适配器事件
- **任务状态总览**：集中查看运行中、等待输入、已完成和异常任务
- **智能提示检测**：自动识别命令行工具执行过程中的输入请求（yes/no 确认、自定义输入等）
- **单入口多出口**：在 GUI 中查看上下文、直接回复、接管任务、回到原始工具或打开对应工作区
- **账号 / provider 切换兼容**：支持识别 CC Switch / Claude Code Switch 类工具，先切换账号或 provider，再由 CodePanion 接管实际 CLI 任务
